...
const getBucketColor = (shareNorm) => {
  const eased = Math.pow(Math.max(0, Math.min(1, shareNorm)), 0.6);
  if (eased <= 0.55) return mixHexColor('#2563eb', '#7c3aed', eased / 0.55);
  return mixHexColor('#7c3aed', '#e11d48', (eased - 0.55) / 0.45);
};
...
const applyDensityStyle = (points, minRadius, maxRadius, minOpacity, maxOpacity, selectedChannel) => {
  ...
  const shares = selectedChannel ? points.map((point) => Math.pow(point.selectedShare || 0, 0.7)) : [0];
  const minShare = Math.min(...shares);
  const maxShare = Math.max(...shares);

  return points.map((point) => {
    const densityNorm = normalize(Math.log1p(point.totalCount), minDensity, maxDensity, 0, 1);
    const shareNorm = selectedChannel
      ? normalize(Math.pow(point.selectedShare || 0, 0.7), minShare, maxShare, 0, 1)
      : 0;
    return {
      ...point,
      radius: normalize(densityNorm, 0, 1, minRadius, maxRadius),
      opacity: normalize(densityNorm, 0, 1, minOpacity, maxOpacity),
      fill: selectedChannel ? getBucketColor(shareNorm) : '#3b82f6',
    };
  });
};
...
const ScatterChartCanvas = ({ series, xAxisLabel, yAxisLabel, preset }) => {
  const renderDot = (props) => {
    ...
    return (
      <circle
        ...
        strokeWidth={preset.dotStrokeWidth}
      />
    );
  };
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ScatterChart margin={preset.chartMargin}>
        ...
        />
        <Scatter data={series} shape={renderDot} isAnimationActive={false} />
      </ScatterChart>
    </ResponsiveContainer>
  );
};
...
const ScatterPlotView = ({
  ...
  const series = useMemo(
    () => applyDensityStyle(buckets, 3, 13, 0.7, 1, selectedChannel),
  ...
  const chartCanvas = (
    <div style={{ width: '100%', display: 'flex', justifyContent: 'center', flex: '1 1 auto', minHeight: 0 }}>
      ...
      >
        <ScatterChartCanvas series={series} xAxisLabel={xAxisLabel} yAxisLabel={yAxisLabel} preset={chartPreset} />
      </div>
    </div>
  );
