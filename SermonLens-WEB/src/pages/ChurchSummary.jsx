...
// Data imports
import sermonData from '../data/sermonData.json';
...
const ChurchSummary = () => {
  ...
  useEffect(() => {
    ...
    const loadWcIndex = async () => {
      try {
        const res = await fetch('/tfidf/index.json');
        const data = await res.json();
        setWordcloudIndex(data);
        if (data.churches) {
          data.churches.forEach(c => keys.add(c));
        }
      } catch (e) {
        console.error("Failed to load tfidf index:", e);
      } finally {
  ...
  useEffect(() => {
    const fetchChurchWc = async () => {
      ...
      try {
        const res = await fetch(`/tfidf/church/${selectedChurch}.json`);
        const data = await res.json();
  ...
  useEffect(() => {
    const fetchSermonWcs = async () => {
      ...
      const promises = paginatedSermons.map(async (sermonId) => {
        ...
        try {
          const res = await fetch(`/tfidf/sermon/${selectedChurch}/${sermonId}.json`);
                                ...
                                <div style={{ position: 'relative', zIndex: 1 }} className="flex items-center justify-between gap-4">
                                  ...
                                  <div className="flex items-center gap-3 flex-wrap">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                      {item.keywords.slice(0, 3).map((w, i) => (
                                        <span 
                                          key={i} 
                                          style={{
                                            fontSize: '0.8rem',
